...
class MaxQAttenLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        action_pred_loss = F.kl_div(
            ...
            reduction='none'
        ).sum(dim=-1)  # (bs, t+1, actual, predictor)
        # 在 mispred_rewards 中，对于同一小组 (atten_score 大), 鼓励预测正确；对于不同小组 (atten_score 小)，鼓励预测错误 (表明存在探索)
        mispred_rewards = - (atten_score_out.detach() * action_pred_loss.detach()).sum(dim=(2,3))  # (bs, t+1)
        ...
        targets = build_td_lambda_targets(rewards + mispred_rewards[:, :-1].unsqueeze(2), terminated, mask, target_max_qvals,
        ...
        atten_score = atten_score.reshape(-1, self.n_agents, self.n_agents)
        # 用以下 loss 来约束 attention score
        # 传递性 loss
        atten_transitivity_loss = th_utils.transitivity_loss(atten_score)
        # 二值化 loss
        atten_binarization_loss = th_utils.binarization_loss(atten_score)
        # 对称性 loss
        atten_symmetry_loss = th_utils.symmetry_loss(atten_score)
        # 出度限制
        atten_degree_constraint_loss = th_utils.degree_constraint_loss(atten_score, self.args.min_atten_degree, self.args.max_atten_degree)
        ...
        loss =  self.args.qmix_loss * qmix_loss + self.args.central_loss * central_loss + comm_loss + atten_loss + action_pred_loss
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat("mispred_reward", (mispred_rewards[:, :-1] * mask[:,:,0]).sum().item()/(mask_elems), t_env)
