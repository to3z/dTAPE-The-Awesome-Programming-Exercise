...
from modules.comm.information_bottleneck_atten_comm import IBAttenComm
...
class CateBAttenCommFMAC:
	def __init__(self, scheme, groups, args):
		...
		assert self.args.comm_method == 'information_bottleneck_atten'
		self.comm = IBAttenComm(input_shape_for_comm, args)
	...
	def forward(self, ep_batch, t, test_mode=False):
		...
		(mu, sigma), messages, atten_score, pred_actions = self._communicate(ep_batch.batch_size, agent_inputs)
		messages = messages.view(ep_batch.batch_size * self.n_agents, -1)
		# messages: (bs*n_agents, n_agents*comm_embed_dim)
		agent_inputs = th.cat([agent_inputs, messages], dim=1)
		logits = self._logits(ep_batch.batch_size, agent_inputs)
	...
	def _build_agents(self, input_shape):
		self.agent = agent_REGISTRY[self.args.critic_agent](input_shape, self.args)
	...
	def _communicate(self, bs, inputs):
		"""
		agent 之间通信，通过 attention 机制决定信息传递的强度
		Args:
			bs: batch size
			inputs: (bs*n_agents, input_dim)
		Returns:
			mu: (bs, receiver, sender, comm_embed_dim)
			sigma: (bs, receiver, sender, comm_embed_dim)
			message: (bs, receiver, sender, comm_embed_dim)
